...
class Tar {
	...
	if(!isreg($stat))
        ...
        if($read != $stat['size']) {
            print("The size of $file changed while reading, archive corrupted. read $read expected ".$stat['size']."\n");
        }
    }
   ...
   public function create($file = '') {
	...
	switch($stat['mode'] & S_IFMT) {
	    ...
	    default: $typeflag = '0'; printf("unknown file mode for $name: %o", $stat['mode']);
	}
	...
	if($name[0] == '/') $name = ".$name";
        ...
        if ($namelen > 100) {
            ...
            if (strlen($file) > 100 || strlen($dir) > 155) {
                ...
                $this->writeFileHeader('././@LongLink', ['uid' => 0, 'gid' => 0, 'perm' => S_IFLLL, 'size' => $namelen, 'mtime' => 0]);
        ...
        $data = pack("a100a8a8a8a12A12", $name, $perm, $uid, $gid, $size, $mtime)."        ".pack("a1a100a6a2a32a32a8a8a155a12", $typeflag, $linkname, 'ustar', '00', '', '', sprintf("%06o ", $stat['rdev'] >> 8),  sprintf("%06o ", $stat['rdev'] & 255), $prefix, "");
        for ($i = 0, $chks = 0; $i < 512; $i++)
            $chks += ord($data[$i]);
        $data = substr_replace($data, pack("a8", sprintf("%6o ", $chks)), 148, 8);
        $this->writebytes($data);
    }

    function recursive_add($directory) {
